...
# VGG16の前処理（tf->Caffeカラースケールに）
def convert_to_caffe_colorscale(input_tensor):
    assert K.ndim(input_tensor) == 4
    # 0～255に戻す
    x = (input_tensor + 1) * 127.5
    # RGB -> BGR
    x = x[:, :, :, ::-1]
    # 平均シフト
    mean_np = np.array([103.939, 116.779, 123.68], np.float32).reshape(1,1,1,-1)
    return x - K.variable(mean_np)
