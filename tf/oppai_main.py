...
def create_train_pconv_unet():
    ...
    conv1, mask1 = conv_bn_relu(input_image, input_mask, 
                                filters=32, kernel_size=3, downsampling=1, reps=2) # 448x336
    conv2, mask2 = conv_bn_relu(conv1, mask1,
                                filters=64, kernel_size=11, downsampling=7, reps=1)
    ...
    img, mask = conv_bn_relu(img, mask,
                             filters=32, kernel_size=11, upsampling=7, reps=1,
                             concat_img=conv1, concat_mask=mask1) # 448x336
    img, mask = conv_bn_relu(img, mask,
                             filters=32, kernel_size=3, upsampling=1, reps=2)
    img, mask = conv_bn_relu(img, mask,
                             filters=3, kernel_size=1, reps=1, act="tanh") # 出力
    ...
    vgg_in_pred = layers.AveragePooling2D(2)(img)
    vgg_in_groundtruth = layers.AveragePooling2D(2)(input_groundtruth)
    vgg_in_comp = layers.AveragePooling2D(2)(y_comp)
    # Caffeカラースケールに変換
    vgg_in_pred = layers.Lambda(convert_caffe_color_space)(vgg_in_pred)
    vgg_in_groundtruth = layers.Lambda(convert_caffe_color_space)(vgg_in_groundtruth)
    vgg_in_comp = layers.Lambda(convert_caffe_color_space)(vgg_in_comp)
    # vggの特徴量
    vgg_pred_1, vgg_pred_2, vgg_pred_3 = extract_vgg_features(vgg_in_pred, (224,168,3), 0)
    vgg_true_1, vgg_true_2, vgg_true_3 = extract_vgg_features(vgg_in_groundtruth, (224,168,3), 1)
    vgg_comp_1, vgg_comp_2, vgg_comp_3 = extract_vgg_features(vgg_in_comp, (224,168,3), 2)
    # 画像＋損失
    ...
    while True:
        ...
        for i in indices:
            ...
            if len(image_cache) == batch_size:
                ...
                print(np.min(batch_gt), np.max(batch_gt), np.min(batch_mask), np.max(batch_mask))
...
def train():
    ...
    # あとで消す
    train_ind = train_ind[:1024]
    test_ind = test_ind[:128]

    model = create_train_pconv_unet()
    model.summary()
    model.compile(tf.train.RMSPropOptimizer(1e-10), identity_loss, [PSNR])
    ...
    model.fit_generator(data_generator(data, train_ind, batch_size, True),
                        ...
                        callbacks=[cb], epochs=1, max_queue_size=1)
