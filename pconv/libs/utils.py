...
def compare_result_images(censored_images, predicted_images, ground_truth, mappers, file_path):
    ...
    img = (np.transpose(tx.numpy(), [1, 2, 0]) * 255.0).astype(np.uint8)
    ...
    cv2.putText(expand_img, "Censored / Pred / Ground Truth", (100, 10), cv2.FONT_HERSHEY_SIMPLEX, 1.5, 255)
    for i in range(3):
        cv2.putText(expand_img, mappers[i]["json"]["original_url"], (10*266*i, 256*3+70), cv2.FONT_HERSHEY_SIMPLEX, 1, 255)
    cv2.imwrite("sampling/"+filepath, expand_img)
